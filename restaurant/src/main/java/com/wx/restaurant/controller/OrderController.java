...
public class OrderController {
    ...
    /*
    *
    * Commit by Wei Chengxi, at 12/11/2020
    * for repay
    * */
    @GetMapping("/balance/find")
    public Map<String,Object> balanceFind(@RequestParam("order_id") String orderId){
        Map<String,Object> result=new HashMap<>();
        try {
            System.out.println(">>>>>>>>>>>>>>>>>调取用户再支付接口");
            Balance balance=balanceService.balanceFind(orderId);
            balanceService.balanceUpdate(balance.getId());
            List<ShopCart> shopCartList=balanceService.shopCartList(orderId);
            result.put("status", StatusEnum.SUCCESS.getIndex());
            result.put("balance_status",balance.getBalanceStatus());
            result.put("dining_id",orderId);
            result.put("shop_cart",shopCartList);
            result.put("sumMonney",balance.getTotalPrice());
        }catch (Exception e){
            result.put("status",StatusEnum.FAIL.getIndex());
            result.put("errmsg", "付款失败");
        }
